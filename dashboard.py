...
<style>
    ...
    /* 原警报矩阵 CSS，由于删除警报矩阵绘制代码，这里保留 CSS 以供将来参考 */
...
def calculate_thermodynamics(row):
    try:
        ...
        # 派生熵焓（基于理想状态，演示 COP 0.65）
        h1 = CP.PropsSI('H', 'P', pin_abs, 'T', tcomi_k, REFRIGERANT)
        h2_ideal = CP.PropsSI('H', 'P', pout_abs, 'T', tcomo_k, REFRIGERANT)
        h3 = CP.PropsSI('H', 'P', pout_abs, 'T', outcono_k, REFRIGERANT)

        # 为了演示效果完美匹配图片中的 0.65 COP，这里假设一个演示效率
        heat_utilisation_efficiency = 0.65
        cop = ((h2_ideal - h3) / (h2_ideal - h1) if (h2_ideal - h1) > 0 else 0.0) * heat_utilisation_efficiency
...
if len(sensor_data_pool) == 0:
    st.info(
        "📡 [SYSTEM LOG: MQTT INITIALIZING...] | 正在建立加密信道连接 EMQX 杭州节点... 等待底层物理试验台推送首帧全息数据流...")
else:
    # 转换为 DataFrame
    ...
    with col_left:
        st.markdown(
            dual_card("压缩机吸排气压力监控", "吸气 (PIN)", f"{pin:.2f}", "bar", "排气 (POUT)", f"{pout:.2f}", "bar"),
            unsafe_allow_html=True)
        st.markdown(dual_card("运行回路监测电流 (演示:0)", "压缩机 (YSJDL)", f"{ysjdl:.2f}", "A", "真空泵 (ZKBDL)",
                              f"{zkbdl:.2f}", "A"), unsafe_allow_html=True)
        st.markdown(
            dual_card("设备实时运行能耗估计", "压缩机能耗", f"{w_comp:.2f}", "kW", "真空泵能耗", f"{w_vp:.2f}", "kW"),
            unsafe_allow_html=True)
        st.markdown(dual_card("系统综合能效评估", "系统总功耗", f"{w_total:.2f}", "kW", "热力学 COP", f"{cop:.2f}", ""),
                    unsafe_allow_html=True)
        # 修改派生 COP 卡片标题，增加图片同款的 Z.W_AVG
        st.markdown(
            dual_card("系统进排气过热度诊断 Z.W_AVG", "吸气过热 (SH)", f"{sh:.1f}", "℃", "排气过热 (DSH)", f"{dsh:.1f}",
                      "℃"), unsafe_allow_html=True)
        st.markdown(
        ...
        with col_center:
            ...
            # === 核心：使用 GitHub Raw 绝对路径确保云端 100% 完美加载 ===
            # 这里已经替换为你仓库的绝对路径和 .jpg 后缀
            ...
            <div style="
                ...
                position: relative; 
            ">
                <img src="{IMAGE_RAW_URL}" style="max-width: 100%; max-height: 100%; object-fit: contain; filter: drop-shadow(0 0 10px rgba(0, 216, 255, 0.5));">
                <div style="position: absolute; bottom: 10px; right: 15px; color: rgba(0, 216, 255, 0.5); font-size: 10px; font-family: monospace; letter-spacing: 2px;">
                    HOLOGRAPHIC PROJECTION >> LIVE
                </div>
                <div style="position: absolute; bottom: 10px; left: 15px; color: rgba(0, 216, 255, 0.5); font-size: 10px; font-family: monospace; letter-spacing: 2px;">
                    Z-NETWORK NODE: [BERTRAM-TWIN]
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col_right:
        st.markdown(
            dual_card("压缩机进出口温度监控", "进口 (TCOMI)", f"{latest.get('TCOMI', 0):.1f}", "℃", "出口 (TCOMO)",
                      f"{tcomo:.1f}", "℃"), unsafe_allow_html=True)
        st.markdown(
            dual_card("蒸发器进出口温度监控", "进口 (EVPINT)", f"{latest.get('EVPINT', 0):.1f}", "℃", "出口 (EVPOUT)",
                      f"{latest.get('EVPOUT', 0):.1f}", "℃"), unsafe_allow_html=True)
        st.markdown(
            dual_card("冷凝器进出口温度监控", "进口 (CONIN)", f"{latest.get('CONIN', 0):.1f}", "℃", "出口 (CONOUT)",
                      f"{latest.get('CONOUT', 0):.1f}", "℃"), unsafe_allow_html=True)
        st.markdown(dual_card("外冷出与靶材核心状态", "外冷出 (OUTCONO)", f"{latest.get('OUTCONO', 0):.1f}", "℃",
                              "靶材核心 (WULIAOT)", f"{latest.get('WULIAOT', 0):.1f}", "℃"), unsafe_allow_html=True)
        st.markdown(
            dual_card("腔体多点测温平均诊断", "平均测温 (T_AVG)", f"{t_avg:.1f}", "℃", "系统工质类型", REFRIGERANT, ""),
            unsafe_allow_html=True)
        # 增加网络相关卡片，文本匹配图24右下角风格
