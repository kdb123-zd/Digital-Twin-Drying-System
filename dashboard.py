...
if len(sensor_data_pool) == 0:
    ...
    with col_left:
        ...
        st.markdown(dual_card("系统综合能效评估", "系统总功耗", f"{w_total:.2f}", "kW", "热力学 COP", f"{cop:.2f}", ""),
                    unsafe_allow_html=True)
        st.markdown(
            dual_card("系统进排气过热度诊断", "吸气过热 (SH)", f"{sh:.1f}", "℃", "排气过热 (DSH)", f"{dsh:.1f}", "℃"),
            unsafe_allow_html=True)
        st.markdown(
    ...
    with col_center:
        ...
        <div class="alert-matrix">
            ...
            <div class="alert-box {pin_cls}"><div class="led-bulb {pin_led}"></div><div class="alert-title">低压吸气(绝压)</div><div class="alert-val" style="color: {'#EF4444' if pin < LOW_PIN_LIMIT else '#10B981'};">{pin_abs_ui:.2f} bar</div></div>
